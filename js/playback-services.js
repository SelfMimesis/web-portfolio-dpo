...
export function createPlaybackServices(panel,{mobile=false}={}) {
  ...
  if(!mobile){
    timeline.to(panel.querySelector('.playback-copy'),{opacity:.12,duration:.2},0)
      .to(panel.querySelector('.playback-photo'),{opacity:.16,duration:.2},0);
    ...
    timeline.to(panel.querySelector('.playback-copy'),{opacity:1,duration:.2},4.14)
      .to(panel.querySelector('.playback-photo'),{opacity:1,duration:.2},4.14);
  }else{
  ...
  function draw(progress){
    ...
    paintGradient(tick);
    cards.forEach((card,i)=>{
      const phase=mobile?progress:Math.max(0,Math.min(1,(progress*4.34-i-.14)/.82));
